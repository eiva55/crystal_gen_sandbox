...
if version.parse(ase.__version__) > version.parse("3.22.1"):
    ...
    from ase.constraints import FrechetCellFilter as CellFilter
...
def stepwise_relax(
    ...
    :param optimizer: a local optimization algorithm, default BFGS
    :param hydrostatic_strain: if do isometrically cell-scaled relaxation, default True
    :param fmax: the max force per atom (unit as defined by the calculator), default 0.05
...
def single_run(
    ...
    if relax_calculator is None:
        relax_calculator = build_mace_calculator()
...
def build_mace_calculator() -> Calculator:
    """Return a MACE ASE calculator."""
    model_mace_mpa = '/home/users/ntu/shuya003/WyckoffTransformer/cryspr/mace-mpa-0-medium.model'
    device_global = 'cuda' if torch.cuda.is_available() else 'cpu'

    print('Using Mace personal model')
    try:
        calc = MACECalculator(model_paths=model_mace_mpa,device=device_global,enable_cueq=True, default_dtype="float64")
        print('Using Mace with cueq')
    except:
        calc = MACECalculator(model_paths=model_mace_mpa,device=device_global,enable_cueq=False, default_dtype="float64")
...
def func_run(
    ...
    # FIX: Initialize formula as None so it exists even if all trials fail
    formula = None 
    ...
    for i_trial in range(n_trial_each_wyckoff_gene):
        ...
        # This will now safely assign if successful
        ...
        try:
            ...
            atoms_relaxed = single_run(
                ...
                fmax=0.01,  # for consistency with previous
    ...
    if len(atoms_relaxed_dict) == 0:
        ...
        # FIX: Add graceful logging if all trials skipped/failed
        with open(f"{rootdir}/cryspr.log", mode='a+') as f:
            f.write(
                f"[{now()}] CrySPR Warning: All {n_trial_each_wyckoff_gene} trials failed/skipped for {model}-{id_gene}\n"
            )
    else:
    ...
    with open(tmp_csv_file, 'a+') as f:
        # If formula is None, it will write "None" to the CSV rather than crashing
        f.write(
